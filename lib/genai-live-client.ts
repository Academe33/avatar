...
// Augment the GenAILiveClient class type
export interface GenAILiveClient {
  on<K extends keyof LiveClientEventTypes>(event: K, listener: LiveClientEventTypes[K]): this;
  on(event: string, listener: (...args: any[]) => void): this; // Fallback

  off<K extends keyof LiveClientEventTypes>(event: K, listener: LiveClientEventTypes[K]): this;
  off(event: string, listener: (...args: any[]) => void): this; // Fallback

  emit<K extends keyof LiveClientEventTypes>(event: K, ...args: Parameters<LiveClientEventTypes[K]>): boolean;
  emit(event: string, ...args: any[]): boolean; // Fallback for emit
}
